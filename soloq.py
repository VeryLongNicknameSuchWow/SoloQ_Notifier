...
# Either RIOT_ID or PUUID must be provided
RIOT_ID = None
PUUID = None
with contextlib.suppress(KeyError):
    RIOT_ID = config_section["RIOT_ID"]
with contextlib.suppress(KeyError):
    PUUID = config_section["PUUID"]
...
def add_ordinal_suffix(n):
    ...
    return f"{n}{suffixes.get(last_digit, 'th')}"
...
def notify_game_result(account_dto, summoner_dto, data):
    ...
    match_by_id_url = (
        f"https://{WIDE_REGION}.api.riotgames.com/lol/match/v5/matches/{match}"
    )
                        ...
                        "embeds": [
                            {
                                "title": username,
                                "description": (
                                    f"{placement}game {result} {emoji}{rank_message}"
                                ),
                                "color": color,
                                "footer": {"text": f"ID: {numeric_id}"},
                            },
